...
Skipped unless the environment below is set; run with ``pytest -m integration``. The local
stack's ``SdkIntegrationSeeder`` gives this binding its own fixture project (slot 15), with
all three key types on it — every value fixed, so the run is repeatable on demand::
